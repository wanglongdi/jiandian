    ...
    $.fn.mtimer = function(opts){
		...
		var picker = {
			init : function(){
				...
				for(var i=1; i<option.dateNum; i++){
					...
					var ye=y+'-'+m+'-'+d;
					if(i==1){
						dateStr += '<li class="'+sel+'" data-date="明天（'+m+'月'+d+'日）" data-tt="'+ye+'">明天（'+m+'月'+d+'日）</li>';
					}else{
						dateStr += '<li class="'+sel+'" data-date="周'+w+'（'+m+'月'+d+'日）" data-tt="'+ye+'">周'+w+'（'+m+'月'+d+'日）</li>';
					}
			...
			renderHTML : function(){
				...
				var html = '<div class="mt_mask"></div><div id="mtimer" class="mt_poppanel"><div class="mt_panel"><div class="mt_confirm"><a href="javascript:void(0);" class="mt_ok">确定</a> <a href="javascript:void(0);" class="mt_cancel">取消</a></div><div class="mt_body"><div class="mt_date"><ul><li></li><li></li></ul></div><div class="mt_indicate"></div></div></div></div>';
				$(document.body).append(html);
			},
			updateSelected : function(container, iscroll){
				var index = (-iscroll.y) / itemHeight + 3;
			...
			setValue : function(){
				...
				var date = value;
					//time = dateArr[1],
				var dateItem = $('.mt_date li[data-date="'+date+'"]');
